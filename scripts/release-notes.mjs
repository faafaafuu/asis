...
| Другой Linux | \`.AppImage\` |
| Android | \`.apk\` |

Скачайте файл под свою систему и запустите. Командная строка не нужна.

**При первом запуске.** Windows покажет «Windows защитила ваш компьютер» → «Подробнее» →
«Выполнить в любом случае». macOS скажет, что разработчик не подтверждён → откройте
правым кликом → «Открыть», затем разрешите доступ в разделе «Конфиденциальность
