...
- **Windows 10 / 11** — \`.exe\`. Не подписан: SmartScreen предупредит, нажмите «Выполнить в любом случае».
- **macOS** — \`.dmg\`, Intel и Apple Silicon. Первый запуск — правой кнопкой по программе → «Открыть».
- **Linux** — \`.AppImage\` (сделать исполняемым и запустить) или \`.deb\`.
- **Android** — \`.apk\`. Разрешите установку из этого источника, когда телефон спросит.
