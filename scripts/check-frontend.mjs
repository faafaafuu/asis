// Проверка фронтенда без браузера: синтаксис модулей, наличие дизайн-токенов
// и целостность ссылок в HTML. Запуск: `npm run check`.
//
// Это не замена визуальной проверке, а страховка от опечаток: «файл не найден»
// в окне программы стоит дорого.
