"""Собирает гифки для README.

Кадры снимает headless-браузер со страницы `scene.html`, где собран попап и
индикатор из тех же файлов, что и в программе. Номер кадра задаётся ссылкой.

    python scripts/shots/make.py

Нужны Chrome (или Edge) и ffmpeg в PATH. Готовые файлы кладутся в `docs/`.
"""
