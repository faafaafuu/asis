...
При первом запуске откроется окно настройки: там же — живая проверка перехвата. Выделите
любое слово с левым Ctrl, и строка «Проверка перехвата» покажет, дошёл ли жест и удалось
ли получить текст. Дальше окно не нужно: программа сворачивается в трей и ждёт выделения.
Вернуться к нему можно кликом по значку в трее.
