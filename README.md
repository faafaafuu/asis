...
Ноа проверяет каждый модуль перед запуском и запускает только проверенную
версию — регламент в [module_format.md](src-tauri/src/module_format.md).

Готовые модули — на [площадке NOAH](http://84.247.166.53:8795) и в библиотеке
главного окна. Свой модуль публикуется оттуда же: ключ из кабинета автора
вставляется в Ноа → Настройки → Площадка, дальше нейросеть делает
`publish_module`.
