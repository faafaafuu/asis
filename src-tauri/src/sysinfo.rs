...
pub async fn diagnose(app: &AppHandle, question: &str) -> String {
    ...
    let rules = format!(
        "Ты — {name}, помощник на компьютере человека с Windows, и у тебя есть данные \
         ...
         Если есть что поправить — закончи ровно одним вопросом из этих: \
         «Открыть центр обновления?», «Открыть диспетчер устройств?», «Открыть \
         очистку диска?», «Открыть параметры памяти?», «Открыть параметры звука?», \
         «Открыть просмотр событий?», «Открыть службы?». Отвечай ровно на \
    ...
    match provider.advise(&rules, question).await {
        Ok(answer) if !answer.trim().is_empty() => {
            ...
            // Предложили открыть — запомнить: «да» следующей фразой откроет.
            if let Some(target) = offered_target(&answer) {
                crate::planner::offer(target);
            }
            answer
        }
...
/// Что предложили открыть в конце ответа — имя, которое понимает `pc::launch`.
fn offered_target(answer: &str) -> Option<&'static str> {
    let tail = answer.rsplit(['.', '!']).next().unwrap_or(answer).to_lowercase();
    if !tail.contains("открыть") || !tail.trim_end().ends_with('?') {
        return None;
    }
    [
        ("центр обновления", "центр обновления"),
        ("диспетчер устройств", "диспетчер устройств"),
        ("очистку диска", "очистка диска"),
        ("параметры памяти", "память"),
        ("параметры звука", "звук"),
        ("просмотр событий", "просмотр событий"),
        ("службы", "службы"),
    ]
    .iter()
    .find(|(said, _)| tail.contains(said))
    .map(|(_, target)| *target)
}
...
mod tests {
    ...
    #[test]
    fn an_offer_at_the_end_is_remembered() {
        assert_eq!(
            offered_target("Старые драйверы у Killer Wi-Fi. Открыть центр обновления?"),
            Some("центр обновления")
        );
        assert_eq!(offered_target("На C: мало места. Открыть очистку диска?"), Some("очистка диска"));
        assert_eq!(offered_target("Всё в порядке."), None);
    }
