...
///
/// Было пять минут (умолчание Ollama), потом тридцать, потом восемь часов —
/// чтобы после совещания выделенное слово не ждало тринадцати секунд загрузки.
/// Восемь часов оказались другой крайностью: модель весь день держала четыре
/// с половиной гигабайта видеопамяти, в том числе пока на компьютере играли, и
/// оставалась там после выхода из Суфлёра — Ollama живёт отдельно от него.
///
/// Теперь двадцать минут, а ожидание загрузки прячется иначе: когда Ноа зовут
/// по имени или зажимают Alt с пробелом, модель поднимается заранее и успевает
/// лечь в память, пока человек договаривает вопрос (см. `wake_local_model`).
/// При выходе из программы она выгружается сразу (см. `unload_ours`).
pub const KEEP_ALIVE: &str = "20m";
...
pub async fn unload_others(host: &str, keep: &str) {
    ...
    for name in loaded {
        ...
        unload(host, &name).await;
        log::info!("выгружаю из памяти лишнюю модель {name}");
    }
}

/// Выгружает модель из памяти прямо сейчас: нулевой срок жизни.
///
/// Ошибки не важны — это уборка; не вышло, значит, память освободится сама
/// по истечении срока.
pub async fn unload(host: &str, model: &str) {
    let Ok(client) = crate::net::client_builder()
...
/// Выгружает всё, что загрузили мы, — при выходе из программы.
///
/// Ollama — отдельная служба и программу переживает: без этого модель
/// оставалась бы в видеопамяти ещё на срок `KEEP_ALIVE`, уже никому не нужная.
pub async fn unload_ours(host: &str) {
    let ours: Vec<String> = WARMED.lock().unwrap_or_else(|err| err.into_inner()).clone();
    for model in ours {
        unload(host, &model).await;
        log::info!("выход: выгружаю из памяти модель {model}");
    }
}
