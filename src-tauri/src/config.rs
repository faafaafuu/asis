//! Конфигурация приложения: endpoint и ключ AI-провайдера не хардкодятся.
//!
...
pub struct PlatformConfig {
    pub url: String,
    /// Ключ площадки из кабинета автора. Хранится зашифрованным (DPAPI).
    pub token: String,
}

impl Default for PlatformConfig {
    fn default() -> Self {
        Self { url: DEFAULT_PLATFORM_URL.into(), token: String::new() }
    }
}
...
pub struct FoodConfig {
    ...
    /// Сессия браузера FoodPilot, в которой человек уже вошёл в магазин.
    ///
    /// Пусто — Ноа только подбирает и называет сумму. Складывать в корзину без
    /// вошедшего профиля нельзя: магазин уводит нажатие на страницу входа, и
    /// товар молча не добавляется.
    pub session_id: String,
    /// Оплачивать ли заказ самому, не спрашивая человека.
    ///
    /// Оплата идёт способом, сохранённым в самом магазине, и только в сессии
    /// браузера, где человек вошёл. Работает лишь вместе с потолком
    /// `max_order`: без потолка ошибка распознавания или подбора превратилась
    /// бы в списание любой суммы, а с ним худшее, что может случиться, —
    /// заказ не того на сумму, которую человек сам счёл допустимой.
    pub auto_pay: bool,
    /// Сколько Ноа может заплатить сам за сутки, ₽.
    ///
    /// `max_order` — предел одного заказа без подтверждения, этот — всех
...
impl Default for FoodConfig {
    fn default() -> Self {
        Self {
            ...
            // Выключено, пока человек сам не включит: списание денег не должно
            // начаться от обновления программы.
            auto_pay: false,
            daily_limit: 5000,
...
pub struct TriggerConfig {
    ...
    /// (Windows, приложения без поддержки UI Automation).
    pub clipboard_fallback: bool,
    /// Linux: открывать попап по изменению PRIMARY selection, хотя факт нажатия Ctrl
    /// на этом окружении определить нельзя. По умолчанию выключено —
    /// безопасное поведение: лучше не открыть, чем открыть не вовремя.
...
impl Default for TriggerConfig {
    fn default() -> Self {
        Self {
            ...
            // пользователю неоткуда узнать почему. Согласие, которого требует,
...
pub struct VoiceConfig {
    ...
    pub wake_word: bool,
    /// Имя, на которое отзывается помощник — своё у каждого человека.
    ///
    /// По умолчанию «Ноа»: коротко, безударно, редко звучит в обычной речи —
    /// распознавание путает его с обычными словами реже прочих. Своё имя
    /// подхватывается тем же слухом, но не так тонко настроенным: под него
    /// нет таблицы типичных ослышек, только общее правило «на одну-две буквы
    /// мимо». Пустая строка — не значит «без имени», см. `wake_name()`.
    ...
    /// Показывать ли окно с ответом на голосовой вопрос.
    ///
    /// Выключено — Ноа отвечает только голосом, на экране остаётся один
...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            ...
            wake_word: true,
            wake_name: DEFAULT_WAKE_NAME.into(),
...
/// Имя по умолчанию: короткое, безударное, редко встречается как обычное
/// слово в русской речи — из-за этого распознавание путает его с обычными
/// словами реже прочих. Ставится сразу после установки, без выбора.
pub const DEFAULT_WAKE_NAME: &str = "Ноа";

impl VoiceConfig {
    /// Имя, к которому отзывается помощник. Пустая строка в файле настроек
    /// (например, после ручной правки config.json) — это не значит «без
    /// имени», а значит «имя по умолчанию»: без него не собрать ни подсказку
    /// распознаванию, ни то, как помощник представляется в ответах.
    pub fn wake_name(&self) -> &str {
        let name = self.wake_name.trim();
        if name.is_empty() {
            DEFAULT_WAKE_NAME
        } else {
            name
...
impl Config {
    ...
    fn normalize(&mut self) {
        // Онлайн-голоса ещё не написаны, а выбрать их было можно: каждая фраза
        // шла с предупреждением в журнал и всё равно своим голосом.
