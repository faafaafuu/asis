...
async fn order(app: &AppHandle, items: &[crate::food::Wanted], wanted_store: &str) -> String {
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            // Служба заказов — отдельная программа, и чаще всего она просто
            // не запущена. Человеку надо знать именно это: «не отвечает»
            // звучит как поломка, из-за которой он полезет чинить Ноа.
            return if err.contains("не запущена") {
                // Модуль «Магазины» умеет ровно то же, что служба, кроме оплаты:
                // ищет товары и держит список. Если он стоит — человеку не надо
                // ничего поднимать, надо сказать другую фразу.
                if crate::plugins::installed(app).iter().any(|module| module.id == "shops") {
                    "Служба заказов с оплатой не запущена, но искать и собирать список я умею модулем «Магазины». Скажите: «найди молоко» или «добавь молоко в список»."
                        .into()
                } else {
                    "Заказы собирает отдельная служба, и она не запущена. Включить её — в настройках, раздел «Заказы».".into()
                }
            } else {
                format!("Поиск по магазинам не отвечает: {err}")
            };
        }
    };
