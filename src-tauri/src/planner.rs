...
async fn interpret(app: &AppHandle, rules: &str, said: &str) -> Option<serde_json::Value> {
    ...
    let (from, to) = (raw.find('{')?, raw.rfind('}')?);
...
fn add_rules() -> String {
    format!(
        "Ты разбираешь распоряжение о задаче. {}\n\
         Ответь одной строкой JSON и ничем больше: {{\"title\": \"…\", \"due\": \"…\"}}.\n\
         title — что сделать, коротко и без слов «напомни», «запиши», «не забудь».\n\
         due — срок в виде ГГГГ-ММ-ДДTЧЧ:ММ по местному времени, \
         либо пустая строка, если срок не назван.\n\
...
fn time_rules() -> String {
    format!(
        "Человек называет срок задачи. {}\n\
         Ответь одной строкой JSON и ничем больше: {{\"due\": \"…\"}}.\n\
