...
/// Готов ли сервер расшифровки прямо сейчас — без ожидания и без запуска.
///
/// По этому индикатор решает, что показывать после фразы: «думаю» или
/// загрузку. Загрузка бывает долгой, и молчаливое «думаю» на целую минуту
/// выглядело как зависание.
pub fn ready_now() -> bool {
    !STOPPING.load(std::sync::atomic::Ordering::SeqCst) && server_alive()
}
...
fn ensure_server(app: &AppHandle) -> Result<(), String> {
    ...
    command
        ...
        .arg("--best-of")
        .arg("5")
        // Порог «здесь никто не говорит» — как у самой модели по умолчанию.
        // Со строгим 0.35 тихая речь уходила в тишину целиком: микрофон в
        // телефоне или в веб-камере пишет негромко, и помощник молчал в ответ
        // на каждую фразу.
        .arg("--no-speech-thold")
        .arg("0.6")
...
pub async fn transcribe(
    ...
    if echoes_prompt(&text, prompt) {
        ...
        log::info!("расшифровка повторяет подсказку — пропускаю: «{text}»");
        return Ok(String::new());
    }
    if is_hallucination(&text) {
        // В журнал — коротко. Выдумки на шорох случаются часто, но без этой
        // строки пропавшая фраза выглядела в журнале как «тишина»: восемь
        // обращений подряд исчезли, и понять почему было не по чему.
        let short: String = text.chars().take(60).collect();
        log::info!("расшифровка похожа на выдумку модели — пропускаю: «{short}»");
        return Ok(String::new());
    }
    Ok(text)
}
