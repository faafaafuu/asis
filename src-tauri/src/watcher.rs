...
pub fn spawn(app: &AppHandle) -> Integration {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            // забирает фокус, и клавиатурные события до него не доходят.
            //
            ...
            // Пока программа говорит или идёт разговор, окно живо, и минута
            // отсчитывается от последнего слова, а не от открытия. Без этого
            // длинный ответ, дочитанный вслух, съедал всю минуту, и окно
            // закрывалось через секунды после конца чтения — ровно тогда, когда
            // человек собирался спросить следующее.
