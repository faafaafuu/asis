...
/// убрать индикатор, а ответ, который ещё думается, не произносить.
#[cfg(desktop)]
fn cancel_everything(app: &tauri::AppHandle) {
    ...
    // Окна Esc голоса не закрывает — ни ответа, ни какого другого. Человек
    // просит Ноа замолчать, а не убрать то, что он читает: закрыть окно он
    // может сам. Окно объяснения выделенного слова по-прежнему закрывается
    // своим Esc, когда оно в фокусе, — это его собственная клавиша.
...
fn wake_hint(name: &str) -> String {
    ...
    //
    // И без образца обращения. Образец «{name}, …» распознавание в тишине
    // повторяло дословно — с кавычками и многоточием, — и голое имя без
    // вопроса значило «позвали»: помощник открывался сам, когда его никто не
    // звал. Подсказке достаточно одного имени, чтобы писать его правильно.
    format!("{name} — имя голосового помощника.")
}

/// Отголосок подсказки, а не зов: имя и многоточие, без единого слова после.
///
/// Так распознавание заполняет тишину — повторяет то, чем его подсказали.
/// Живое «Чувак... открой браузер» сюда не попадает: после имени есть слова.
#[cfg(desktop)]
fn echo_of_hint(text: &str, question: &str) -> bool {
    let trailed = text.contains('…') || text.contains("...");
    trailed && !question.chars().any(char::is_alphanumeric)
}
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            for heard in phrases {
                ...
                if echo_of_hint(&text, &question) {
                    log::info!("«{text}» — отголосок подсказки в тишине, не зов");
                    continue;
                }
...
fn warm_model_after_speech(app: &tauri::AppHandle) {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Голос поднимается заранее, а не на первой фразе. Раньше первое
            // «привет» ждало холодного старта Silero по полминуты — Ноа
            // «думала» над ответом, который был готов за секунду, — а если
            // сервер не успевал, фраза уходила запасным голосом, и дальше голоса
            // чередовались. Порядок тот же: сперва распознавание, потом голос,
            // чтобы они не толкались за память на старте.
            voice::warm(&app);
            wake_local_model(&app);
        })
        .ok();
...
mod tests {
    ...
    fn an_echo_of_the_hint_is_not_a_call() {
        // Так выглядели самопроизвольные пробуждения в журнале.
        let heard = "«Чувак, …».";
        let question = wake_split(heard, "Чувак").unwrap_or_default();
        assert!(echo_of_hint(heard, &question), "имя с многоточием — отголосок");

        // Живая речь с паузой — зов.
        let heard = "Чувак... открой браузер";
        let question = wake_split(heard, "Чувак").unwrap_or_default();
        assert!(!echo_of_hint(heard, &question), "после имени есть слова");

        // Просто имя без многоточия — тоже зов: позвали и ждут.
        let heard = "Чувак!";
        let question = wake_split(heard, "Чувак").unwrap_or_default();
        assert!(!echo_of_hint(heard, &question));
    }
